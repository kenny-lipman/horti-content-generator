...
import { getDashboardStats, getRecentImages, getTopImageType } from "@/lib/data/billing"
...
export default async function DashboardPage() {
  ...
  const [stats, recentImages, notifications, topImageType] = await Promise.all([
    getDashboardStats(organizationId),
    getRecentImages(organizationId, 8),
    getNotifications(organizationId, { limit: 5 }),
    getTopImageType(organizationId),
  ])
